...
avg_bid_wide <- avg_bid_table_clean %>%
  select(ffs_quartile, market_type, avg_bid) %>%
  pivot_wider(
...
######################
# nearest neighbor matching (1-to-1) with inverse variance cooks.distance
######################
...
mean.treat1 <- hhi_weight %>% filter(treat==1) %>%
    select(avg_bid, ipw) %>% summarise(mean_y = weighted.mean(avg_bid, ipw))
