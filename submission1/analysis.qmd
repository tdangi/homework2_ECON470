---
title: ECON 470 Homework 2 Submission 3
author: Tammy Dang
format: pdf
---
...
#load library
library(pacman)
p_load(tidyverse, ggplot2, dplyr, lubridate, stringr, readxl, data.table, gdata, scales, kableExtra, Matching)
...
ggplot(ma_data %>% filter(year == 2014), aes(x=bid))+
...
#| echo: false
# 2018 plan bids
ggplot(ma_data %>% filter(year == 2018), aes(x=bid))+
...
Construct this ma_hhi2 for part 2:
```{r}
#| include = FALSE
...
hhi_2018 <- hhi_2018 %>%
  mutate(
    market_type = case_when(
      hhi_ma <= hhi_cutoffs[1] ~ "competitive (control)",
      hhi_ma >= hhi_cutoffs[2] ~ "concentrated (treated)",
      TRUE ~ NA_character_
    )
  )
...
hhi_2018 <- hhi_2018 %>%
  ...
  mutate(
    treat = ifelse(market_type == "concentrated (treated)", 1, 0)
  )
...
avg_bid_by_market <- hhi_2018 %>%
  mutate(
    market_type = ifelse(treat == 1, "Concentrated (treated)", "Competitive (control)")
  ) %>%
...
```{r}
#| echo = FALSE
#continue using filter hhi_2018
#national quartile cutoffs for average FFS costs
ffs_q <- quantile(
    hhi_2018$ffs_cost,
...
# create 4 quartile indicator variables. Each indicator equals 1 if the country falls in that quartile, 0 otherwise
hhi_2018 <- hhi_2018 %>%
    mutate(
        ffs_q1 = ifelse(ffs_cost <= ffs_q[1], 1, 0),
        ffs_q2 = ifelse(ffs_cost >  ffs_q[1] & ffs_cost <= ffs_q[2], 1, 0),
        ffs_q3 = ifelse(ffs_cost >  ffs_q[2] & ffs_cost <= ffs_q[3], 1, 0),
        ffs_q4 = ifelse(ffs_cost >  ffs_q[3], 1, 0)
  )
# check
#table(hhi_2018$ffs_q1 + hhi_2018$ffs_q2 + hhi_2018$ffs_q3 + hhi_2018$ffs_q4) #we want all 1s

# create quartile label
hhi_2018 <- hhi_2018 %>%
    mutate(
        ffs_quartile = case_when(
            ffs_q1 == 1 ~ "Q1",
            ffs_q2 == 1 ~ "Q2",
            ffs_q3 == 1 ~ "Q3",
            ffs_q4 == 1 ~ "Q4"
...
# create average bid table
avg_bid_table <- hhi_2018 %>%
...
# drop NA quartile and reshape to treated vs control side-by side
avg_bid_table_clean <- avg_bid_table %>%
  filter(!is.na(ffs_quartile))

avg_bid_wide <- avg_bid_table_clean %>%
  dplyr::select(ffs_quartile, market_type, avg_bid) %>%
  tidyr::pivot_wider(
...
### Nearest neighbor matching (1-to-1) with inverse variance
```{r}
#| echo: false
######################
# nearest neighbor matching (1-to-1) with inverse variance 
######################
# treat 1 = concentrated, 0 = competitive
nn.est1 <- Match(Y = hhi_match$avg_bid,
                Tr = hhi_match$treat,
                X = cbind(
                        hhi_match$ffs_q1,
                        hhi_match$ffs_q2,
...
### nearest neighbor matching with mahalnobis
```{r}
#| echo: false
nn.est2 <- Match(Y = hhi_match$avg_bid,
                Tr = hhi_match$treat,
                X = cbind(
                        hhi_match$ffs_q1,
                        hhi_match$ffs_q2,
...
### inverse propoensity weighting
```{r}
#| echo: false
logit.model <- glm(treat~cbind(
                        hhi_match$ffs_q1,
                        hhi_match$ffs_q2,
                        hhi_match$ffs_q3,
                        hhi_match$ffs_q4
                         ), family = binomial, data=hhi_match)
ps <- fitted(logit.model) #propensity score to include weight

hhi_weight <- hhi_match %>%
...
mean.treat1 <- hhi_weight %>% filter(treat==1) %>%
    dplyr::select(avg_bid, ipw) %>% summarise(mean_y = weighted.mean(avg_bid, ipw))
mean.treat0 <- hhi_weight %>% filter(treat==0) %>%
    dplyr::select(avg_bid, ipw) %>% summarise(mean_y = weighted.mean(avg_bid, ipw))

ate_ipw = mean.treat1 - mean.treat0

#reweighted ate result
ate_ipw
```

### SLRegression w/ the 4 quartiles
```{r}
#| echo: false
#outcome if you're treated
reg1.data <- hhi_match %>% filter(treat==1)
reg1 <- lm(avg_bid ~ factor(ffs_quartile), data=reg1.data)

# outcome if you're in control group
reg0.data <- hhi_match%>% filter(treat==0)
reg0 <- lm(avg_bid ~ factor(ffs_quartile), data = reg0.data)


#2 predictors for every person then minus both for ATE
pred1 <- predict(reg1, new=hhi_weight)
pred0 <- predict(reg0, new=hhi_weight)
ATE <- pred1-pred0
summary(ATE)
...
> The results of each ATE, ipw, or estimates are identical to each other.
...
I will use nearest neighbor matching with malhanobis. The result of this estimate is less robust in terms of unweighted matched number of observations in comparison to when we only match on FFS quartile. In addition, the results are not statistically significant, with a wider standard error overall.
```{r}
#| echo: false
# avg_bid ~ ffs_cost  + avg_eligibles, treat = treat
nn.est_mal <- Match(Y = hhi_match$avg_bid,
                    Tr = hhi_match$treat,
                    X = cbind(hhi_match$ffs_cost, hhi_match$avg_eligibles),
                    M = 1,
                    Weight = 2,
                    estimand = "ATE"
                    )
